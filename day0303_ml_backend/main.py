...
def get_feature(df):
    ...
    for point in df.trajectory[:-1].split(';'):
        point = point.split(',')
        points.append( ( ( (float(point[0]) - 3)/7, (float(point[1]) - 7)/13), float(point[2])) )
...
def make_train_set():
    dump_path = os.path.join(cache, 'train.csv')
...
if __name__ == '__main__':
    ...
    #
    train_x, test_x, train_y, test_y = train_test_split(training_data, label, test_size=0.1, random_state=0)
    #
    # # lgb
    ...
    print(gbm.best_score.values())

    gbm.save_model("model/gbdt.clf")

    #
    # print('Plotting feature importances...')
    # ax = lgb.plot_importance(gbm, max_num_features=10)
    # plt.savefig("picture/feature-importance.png")
    #
    # print('画出训练结果...')
    # ax = lgb.plot_metric(evals_result, metric='auc')  # metric的值与之前的params里面的值对应
    # plt.savefig("picture/train-auc.png")
